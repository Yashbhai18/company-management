...
import { JWT_SECRET, DEV_JWT_SECRET } from '../config/env';

// Derive a 32-byte key from JWT_SECRET for AES-256
const ENCRYPTION_KEY = crypto.createHash('sha256').update(JWT_SECRET).digest();
const DEFAULT_DEV_KEY = DEV_JWT_SECRET ? crypto.createHash('sha256').update(DEV_JWT_SECRET).digest() : ENCRYPTION_KEY;
...
export const decrypt = (text: string): string => {
  ...
  try {
    ...
    if (DEV_JWT_SECRET && JWT_SECRET !== DEV_JWT_SECRET) {
      try {
