...
export function flagToISO(flag: string): string {
  try {
    // Convert the surrogate pairs of regional indicator symbols back into plain uppercase A-Z letters.
    return [...flag].map(char => String.fromCharCode((char.codePointAt(0) || 0) - 127397)).join('');
  } catch {
    return '';
  }
}
